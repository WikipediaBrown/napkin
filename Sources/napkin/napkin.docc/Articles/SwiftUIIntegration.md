...
This matches the example app's convention (see `Examples/LaunchNapkin/AccessibilityIdentifiers.swift`): identifiers are leaf-level, namespaced by feature, and centralized in a single enum so view code and UI tests share a single source of truth.
