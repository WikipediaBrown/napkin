...
- N1. v2.1.5; Apache-2.0; iOS 26/macOS 26+; swift-tools 6.2, language mode v6; sole dependency swift-docc-plugin (docs-only). 2 GitHub stars — young, tiny community. **Ships six Xcode file templates (`Tools/napkin/*.xctemplate`) + `Tools/InstallXcodeTemplates.sh`** — but not RIBs' broader IDE tooling suite (static analysis, runtime integrations) and no runtime leak detector (compile-time isolation + weak-view rule instead); `final actor` interactors, `@MainActor` routers/presenters, native `@Observable` presenters, `Observations`/`AsyncStream` streaming; plain XCTest testing with mocks (see FAQ). *(Corrected 2026-07-16: an earlier revision wrongly said "no code-gen tooling".)*

**VIPER (Task 9 inventory, appended after research wf_f2e18013-f09 completed — all 3-vote-verified 2026-07-15):**
- V1. Canonical source: "Architecting iOS Apps with VIPER" by Jeff Gilbert and Conrad Stoll, objc.io Issue 13 (Architecture), June 2014. Backronym: View, Interactor, Presenter, Entity, **Routing**. Verbatim quotables: "VIPER is an application of Clean Architecture to iOS apps"; Interactor "contains the business logic as specified by a use case"; View "displays what it is told to by the Presenter"; "Entities are never passed from the Interactor to the Presenter"; wireframe owns window/nav/VCs and "together, they describe a route from one screen to the next". [objc.io/issues/13-architecture/viper/]
- V2. Lineage quote (attribute to the 2016 Uber blog, NOT the current README): "When considering alternatives to MVC, we were inspired by the way VIPER could be used as an application of Clean Architecture to iOS apps." [uber.com/blog/new-rider-app-architecture/, Dec 20 2016]
- V3. Current uber/RIBs-iOS README verbatim (live-verified against raw main during review): "MVC, MVP, MVI, MVVM and VIPER are architecture patterns. RIBs is a framework." and "Business logic drives the app, not the view tree." A RIB need not have a view; Presenter/View optional vs VIPER's mandatory V and P.
- V4. Viperit (best-known VIPER library): dormant — latest release 1.5.0 "Xcode 13 compatibility" 2021-09-15, not archived, 513 stars.
- V5. Infinum iOS-VIPER-Xcode-Templates: semi-maintained — latest release v5.0 2023-11-10 on master; feature branches active through 2026-03-27 (SwiftUI module work). The wireframe "actually instantiates the Presenter, Interactor, and View and connects the three via dependency injection" (absorbs RIBs' Builder role).
- V-gap: no third-party boilerplate quote survived verification (make that point structurally, unquoted); no adoption/survey data survived; search-phrasing research returned zero surviving claims twice.
